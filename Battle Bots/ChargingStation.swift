...
var chargingStationNameCount: Int = 0

class ChargingStation: Structure {
    
    override var type: structureType {
        get{return .ChargingStation}
        set{self.type = .ChargingStation}
    }
    
    var chargeAmount: CGFloat = CSChargeAmount
    
    var layer_1 = SKSpriteNode(imageNamed: "ChargingStation_1")
    var layer_2 = SKSpriteNode(imageNamed: "ChargingStation_2")
    
    override init(team: Team, world: World) {
        super.init(team: team, world: world)
        
        chargingStationNameCount += 1
        self.name! += "chargingStation\(chargingStationNameCount)"
        ...
        self.sightRadius = CSSightRadius
